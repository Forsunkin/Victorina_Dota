...
def get_public_matches():
    ...
    for match in re.json():                                         # перебирает все последние матчи, отбирает нужные
...
def post_into_table(obj):
    ...
    cur = conn.cursor()
    cur.execute(f'''INSERT INTO quests(match_id, hero_id, item_1, item_2, item_3, item_4, item_5, item_6, item_neutral)
                VALUES ({obj['match_id']},{obj['hero_id']},{obj['item_0']},{obj['item_1']},{obj['item_2']},
                {obj['item_3']},{obj['item_4']},{obj['item_5']},{obj['item_neutral']})''')
    conn.commit()
# постит  = id матча, id героя, id шмоток
